...
class RegisteredResponsiveImages
{
    ...
    public function __construct(Media $media, string $conversionName = '')
    {
        ...
        $this->files = collect($media->responsive_images[$this->generatedFor]['urls'] ?? [])
            ->map(function (string $fileName) use ($media) { return new ResponsiveImage($fileName, $media); })
            ->filter(function (ResponsiveImage $responsiveImage) { return $responsiveImage->generatedFor() === $this->generatedFor; });
    }

    public function getUrls(): array
    {
        return $this->files
            ->map(function (ResponsiveImage $responsiveImage) { return $responsiveImage->url(); })
            ->values()
            ->toArray();
    }
    ...
    public function getSrcset(): string
    {
        $filesSrcset = $this->files
            ->map(function (ResponsiveImage $responsiveImage) { return "{$responsiveImage->url()} {$responsiveImage->width()}w"; })
            ->implode(', ');
